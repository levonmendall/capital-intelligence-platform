"""Exact dated-futures historical fallback using Databento CME Globex evidence.

No continuous contract or synthetic proxy is accepted here. The caller supplies the
exact dated raw symbol (for example ESM6), and the adapter requests that same symbol
from CME Globex MDP 3.0. Missing entitlement or evidence remains fail-closed so this
fallback cannot change the economic instrument being evaluated.
"""
...
class DatabentoFuturesHistoryProvider:
    def __init__(self, api_key: str | None = None) -> None:
        ...
        probe = DatabentoProvider(api_key=api_key)
        self._configured = probe.configured
