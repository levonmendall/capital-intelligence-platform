...
Failover is scoped to provider + capability + dataset. Candidates must represent the
exact same economic instrument. Fixed-income candidates additionally require explicit
exact-security identity; aggregate TRACE, benchmark yields, auction prices, and proxy
securities can never satisfy individual-bond price evidence.

Databento is intentionally absent from the active all-market redundancy policy. CME is
the intended exact futures primary once its authenticated binding is installed; until
then only actually configured exact-instrument providers may satisfy futures evidence.
"""
...
from providers.redundancy_audit import ProviderCapabilityKey, RedundancyAuditLedger, current_redundancy_ledger
...
_PERMANENT_FOR_CYCLE = frozenset({ProviderFailureClass.AUTHENTICATION_OR_ENTITLEMENT, ProviderFailureClass.ACCESS_OR_CREDIT_CAP, ProviderFailureClass.REQUEST_CONTRACT})
...
class MarketHistoryCandidate:
    ...
    def __post_init__(self) -> None:
        for field_name in ("provider", "capability", "dataset", "provider_symbol", "instrument_identity"):
        ...
        if self.fixed_income and not self.exact_security_identity:
            raise ValueError("fixed-income market-history candidates require exact-security identity")
...
ALL_ASSET_REDUNDANCY_POLICY: dict[str, dict[str, tuple[str, ...]]] = {
    ...
    "crypto": {
        "history": ("alpaca", "coinbase", "kraken", "massive", "twelve_data", "yahoo"),
        "quote_validation": ("coinbase", "kraken"),
    },
    "future": {
        "reference": ("cme", "massive"),
        "history": ("cme", "massive", "yahoo"),
    },
    "fixed_income": {
    ...
    "option": {
        "reference": ("alpaca_indicative", "tradier", "massive"),
        "history": ("alpaca_indicative", "tradier", "massive"),
        "active_chain_validation": ("tradier",),
    },
}
...
class RedundantMarketHistoryRouter:
    ...
    def fetch(self, candidates: Sequence[MarketHistoryCandidate], *, as_of: datetime, minimum_rows: int) -> RoutedMarketHistory:
        ...
        if len(identities) != 1:
            raise RedundantMarketHistoryError("provider failover cannot cross economic-instrument identities")
        if any(item.fixed_income and not item.exact_security_identity for item in candidates):
            raise RedundantMarketHistoryError("fixed-income failover requires exact-security identity")
        ...
        for index, candidate in enumerate(candidates):
            ...
            try:
                rows = _normalize_rows(candidate.loader(), as_of=as_of.astimezone(timezone.utc))
            ...
            first = rows[0]["t"]
            last = rows[-1]["t"]
            source = f"market-history:{key.identifier}:{candidate.provider_symbol}:{first.isoformat()}:{last.isoformat()}:rows={len(rows)}"
        ...
        raise RedundantMarketHistoryError("certified market-history providers could not satisfy the existing evidence contract; " + detail)
