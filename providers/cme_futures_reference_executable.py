...
class CmeExecutableFuturesReferenceProvider(CmeFuturesReferenceProvider):
    """CME-primary reference provider with durable venue/root qualification."""
    ...
    def _records_from_venue_cache(
        ...
        *,
        ...
        if len(contracts) != len(raw_records) or not self._complete(contracts, roots):
            return None
        return tuple(sorted(contracts, key=lambda item: (item.product_code, item.ticker)))
    ...
    def _live_resumable(
        ...
        *,
        ...
        if unresolved:
            ...
            cached_fallback = self._records_from_venue_cache(
                venue="FALLBACK",
                roots=fallback_roots,
                as_of=as_of,
            )
            if cached_fallback is None:
                cause = MassiveMultiAssetError(
                ...
                captured_at = _aware(
                    self._now(), field_name="futures fallback reference captured_at"
                )
                self._write_venue_cache(
                    venue="FALLBACK",
                    roots=fallback_roots,
                    ...
                    contracts=fallback_rows,
                )
                telemetry.extend(dict(item) for item in self.reference_telemetry)
            else:
                fallback_rows = cached_fallback
                telemetry.append(
                    {
                        "provider": "massive",
                        "exchange": "FALLBACK",
                        "mode": "venue_cache",
                        "configured_roots": len(fallback_roots),
                        "covered_roots": len(fallback_roots),
                        "failure_reason": "ok",
                    }
                )
