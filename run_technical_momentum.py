"""Run the point-in-time Technical and Momentum intelligence engine."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the point-in-time technical and momentum engine."
    )
    parser.add_argument(
        "--data-file",
        type=Path,
        default=None,
        help="Immutable technical-momentum-input.v1 provider export.",
    )
    parser.add_argument(
        "--as-of",
        default=None,
    ...
    if args.data_file is not None:
        engine = TechnicalMomentumEngine(
            JSONTechnicalMomentumProvider(args.data_file)
        )
    else:
        engine = build_configured_technical_momentum_engine()
