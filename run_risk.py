"""Run the point-in-time Risk intelligence engine."""
...
from intelligence.risk import JSONRiskProvider, RiskEngine, build_configured_risk_engine
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the point-in-time market-risk intelligence engine."
    )
    parser.add_argument(
        "--data-file",
        type=Path,
        default=None,
        help="Immutable risk-input.v1 provider export.",
    )
    parser.add_argument(
        "--as-of",
        default=None,
    ...
    if args.data_file is not None:
        engine = RiskEngine(JSONRiskProvider(args.data_file))
    else:
        engine = build_configured_risk_engine()
