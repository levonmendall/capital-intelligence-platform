...
class CertifiedEligibleUniversePublication:
    ...
    instrument_approval_identifiers: tuple[tuple[str, str], ...] = ()
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "instrument_approval_identifiers",
            _versions(
                self.instrument_approval_identifiers,
                field_name="instrument_approval_identifiers",
            ),
        )
        unknown_approval_instruments = sorted(
            set(instrument for instrument, _ in self.instrument_approval_identifiers)
            - set(self.eligible_instrument_identifiers)
        )
        if unknown_approval_instruments:
            raise ValueError(
                "instrument approval lineage must reference eligible instruments: "
                f"{unknown_approval_instruments}"
            )

    def approval_identifier_for(self, instrument_identifier: str) -> str | None:
        resolved = _text(
        ...
        return next(
            (
                approval_identifier
                for instrument, approval_identifier
                in self.instrument_approval_identifiers
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "instrument_approval_identifiers": [
                list(item) for item in self.instrument_approval_identifiers
            ],
    ...
    def from_dict(
        ...
        return cls(
            ...
            instrument_approval_identifiers=tuple(
                (str(instrument), str(approval_identifier))
                for instrument, approval_identifier in payload.get(
                    "instrument_approval_identifiers", ()
                )
            ),
            schema_version=str(
...
class SQLiteCertifiedEligibleUniverseStore:
    ...
    def content_hash(self, identifier: str) -> str | None:
        resolved = _text(identifier, field_name="identifier")
        with self._connect() as connection:
            row = connection.execute(
