...
class InstitutionalDatasetActivation:
    ...
    def __post_init__(self) -> None:
        ...
        if self.requested_state in {
            ...
            if self.certification_identifier is None or not self.certification_identifier.strip():
...
def recommended_activation_order() -> tuple[InstitutionalDataset, ...]:
    return tuple(dataset for tier in InstitutionalDataTier for dataset in _TIER_ORDER[tier])
    ...
    *, assessed_at: datetime
    ...
    return tuple(
        InstitutionalDatasetActivation(
            ...
            limitations=("No licensed, configured, and certified provider is active.",),
        )
