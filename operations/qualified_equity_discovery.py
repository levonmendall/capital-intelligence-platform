...
def discover_us_equities(
    *,
    ...
    """Consume exact qualified evidence in production; provider-discover only as owner."""
    ...
    # An injected provider/policy would describe a different evidence generation. A
    # production consumer must never fall back to provider acquisition or silently mix
    # caller-specific evidence with the certified generation.
    if client is not None or sec_provider is not None or policy is not None:
        raise _core.EquityDiscoveryError(
            "production U.S.-equity consumer cannot override qualified evidence"
        ) if hasattr(_core, "EquityDiscoveryError") else EquityDiscoverySnapshotError(
            "production U.S.-equity consumer cannot override qualified evidence"
        )
    ...
    try:
        ...
        raise RuntimeError(f"qualified U.S.-equity discovery snapshot is not ready: {error}") from error
