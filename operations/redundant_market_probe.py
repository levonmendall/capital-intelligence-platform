...
def _candidate_set(record, *, as_of, policy, http_get, eodhd_provider, tradier, massive, twelve, coinbase, kraken, alpaca_crypto_rows):
    ...
    eod = lambda: _legacy._eodhd_rows(record, as_of=as_of, history_days=history_days, provider=eodhd_provider)
    ...
    if record.asset_class in {CandidateAssetClass.US_EQUITY, CandidateAssetClass.US_ETF}:
        ...
        add("eodhd", "international_equity_history", "eodhd-history", record.provider_symbol, eod, configured=eodhd_provider.configured)
        ...
        add("eodhd", "fx_history", "eodhd-history", record.provider_symbol, eod, configured=eodhd_provider.configured)
        ...
        if record.provider_kind == "eodhd" and bool(record.instrument_identifier):
            add("eodhd", "fixed_income_exact_security_history", "eodhd-history", record.provider_symbol, eod, configured=eodhd_provider.configured, fixed_income=True)
