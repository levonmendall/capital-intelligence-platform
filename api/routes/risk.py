"""Read-only Risk intelligence routes."""
...
def latest(settings: ApiSettings = Depends(get_settings)) -> dict[str, Any]:
    ...
    result = None if store is None else store.latest("risk")
    if result is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="risk intelligence is not available",
        )
...
def history(
    ...
    store = _store(settings)
    items = () if store is None else store.history("risk", limit=limit)
    return {
