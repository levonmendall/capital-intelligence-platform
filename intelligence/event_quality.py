    ...
    *,
    ...
    for group in groups:
        representative = max(
            group,
            key=lambda item: (
                _ratio(item.get("reliability"))
                * _ratio(item.get("materiality")),
            ...
            sorted(
                {
                    value
                    for item in group
                    for value in _items(item.get("entities"))
                }
            )
        )
        instruments = tuple(
            sorted(
                {
                    value
                    for item in group
                    for value in _items(item.get("instruments"))
                }
            )
        )
        channels = tuple(
        ...
        explanation = (
            "Worth CIO review: source authority or independent corroboration, "
            "novelty, materiality, and market confirmation meet escalation policy."
            if eligible_for_cio_context
            else (
                "Evaluate and monitor: baseline evidence is sufficient for causal "
                "analysis, but one or more CIO-escalation requirements are not met."
                if eligible_for_analysis
                else (
                    "Monitor intake only: baseline reliability, relevance, "
                    "materiality, or evidence quality is insufficient for causal "
                    "analysis."
                )
            )
        )
    ...
    def __init__(self, path: str | Path) -> None:
        ...
        with sqlite3.connect(self.path) as connection:
            connection.executescript(
                ...
                BEGIN SELECT RAISE(
                    ABORT,
                    'event cluster lineage is append-only'
                ); END;
        ...
        *,
        ...
        with sqlite3.connect(self.path) as connection:
            existing = connection.execute(
                """
                SELECT payload_hash
                FROM event_cluster_assessments
                WHERE identifier = ?
                """,
            ...
            connection.execute(
                """
                INSERT INTO event_cluster_assessments
...
def evaluate_benchmark(path: str | Path) -> dict[str, Any]:
    ...
    for case in annotations:
        ...
        predicted = any(
            item.eligible_for_cio_context for item, _ in results
        )
    ...
    review_state = payload.get("human_review", {}).get(
        "state",
        "missing",
    )
    passed = (
