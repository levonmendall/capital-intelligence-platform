...
The coordinator is downstream of the canonical CIO and construction cycle. It records
which advanced engines had sufficient governed inputs during each real cycle and which
remained fail-closed. Shadow records cannot alter candidates, specialist calculations,
CIO decisions, construction, or execution.
"""
...
class AdvancedIntelligenceShadowCoordinator:
    ...
    def observe_cycle(
        ...
        *,
        ...
        if (
            not cycle_identifier.strip()
            or not code_version.strip()
            or not posture_identifier.strip()
        ):
    ...
    def append(self, snapshot: AdvancedShadowSnapshot) -> str:
        ...
        with sqlite3.connect(self.path) as connection:
            ...
            if existing:
                ...
                raise ValueError(
                    "conflicting advanced-shadow cycle identifier"
                )
