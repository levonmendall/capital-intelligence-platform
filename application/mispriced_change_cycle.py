...
_LOGGER = logging.getLogger("capital_intelligence.joint_portfolio_preview")
...
class MispricedChangeCanonicalCIOCycle(CompoundingCanonicalCIOCycle):
    ...
    def run(self, **kwargs):
        ...
        try:
            ...
            # This preview is additional sizing intelligence, not a new qualification
            # or readiness boundary. Final construction still fails closed after CIO
            # decisions, so preview unavailability must not manufacture a hidden veto.
            _LOGGER.exception(
                "joint portfolio preview unavailable for %s; continuing without a pre-CIO cap",
        ...
        if preview is not None and callable(setter):
            setter(preview)
        try:
            return super().run(
                **{
            ...
            if preview is not None and callable(clearer):
                clearer()
