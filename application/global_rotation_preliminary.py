...
class MemoizedCandidateRiskIntelligenceEngine:
    ...
    def __init__(self, delegate) -> None:
        ...
        self._active_cache: ContextVar[dict[tuple[object, ...], object] | None] = (
            ContextVar(f"global_rotation_candidate_risk_{id(self)}", default=None)
        )
...
class MemoizedJointCandidateIntelligenceEngine:
    ...
    def __init__(self, delegate) -> None:
        self.delegate = delegate
        self._active_cache: ContextVar[dict[tuple[object, ...], object] | None] = (
            ContextVar(f"global_rotation_joint_candidate_{id(self)}", default=None)
        )
    ...
    def assess(self, candidates, risk_assessments, exposure_profiles):
        ...
        if cache is None:
            return self.delegate.assess(candidates, risk_assessments, profiles)
        key = (
        ...
        if result is None:
            result = self.delegate.assess(candidates, risk_assessments, profiles)
            cache[key] = result
        return result
...
def assess_preliminary_global_conviction(
    ...
    *,
    ...
    reconciliation = cio.reconciler.reconcile(
        candidate, specialists, alternative_return=effective_alternative
    )
    ...
    assessment_cap = (
        min(portfolio_cap, candidate.maximum_position_weight, profile.maximum_position_weight)
    ...
    with path_context:
        ...
        assessment_weight = (
            ...
            else min(cio.robust_assessor.policy.minimum_reference_weight, assessment_cap)
        )
