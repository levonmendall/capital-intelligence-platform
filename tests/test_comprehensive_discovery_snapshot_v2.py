...
def test_snapshot_round_trip_preserves_continuity_and_provider_factor_lineage(
    ...
    assert restored.result.as_of == as_of
    assert restored.result.identifier == result.identifier
    assert restored.result.policy_version == result.policy_version
    assert restored.result.manifest_fingerprint == result.manifest_fingerprint
