"""Integration tests for risk scheduling, API, and Personal CIO context."""
...
def test_multi_engine_cycle_persists_seven_results_without_changing_contract(
    tmp_path,
) -> None:
...
def test_risk_api_is_read_only_and_returns_latest_result(tmp_path) -> None:
    ...
    latest = client.get("/v1/risk/latest")
    history = client.get("/v1/risk/history?limit=10")

    assert latest.status_code == 200
    assert latest.json()["engine"] == "risk"
    assert history.status_code == 200
    assert history.json()["total"] == 1
    assert client.post("/v1/risk/latest").status_code == 405
