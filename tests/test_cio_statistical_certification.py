...
def _pair(index: int):
    ...
    expected_candidate = 0.075 + 0.002 * index
    realized_candidate = expected_candidate - 0.01
    packet = {
        ...
        "opportunity": {
            "candidate_expected_return": expected_candidate,
            "marginal_portfolio_improvement": 0.0035 + 0.0002 * index,
            "expected_dollar_value_added": 875.0 + 50.0 * index,
            "current_weight": 0.0,
    ...
    outcome = {
        ...
        "realized_portfolio_return": 0.06,
...
def test_statistical_certification_measures_dollar_value_without_policy_authority():
    report = build_cio_statistical_certification(
        decision_outcome_pairs=tuple(_pair(index) for index in range(12)),
        walk_forward_observations=tuple(_walk(index) for index in range(12)),
        as_of=NOW,
    ...
    assert report.expected_realized_rank_correlation > 0.0
...
def test_generator_input_is_consumed_once_and_small_sample_cannot_certify():
    pairs = (_pair(index) for index in range(2))
    report = build_cio_statistical_certification(
        decision_outcome_pairs=pairs,
        walk_forward_observations=(_walk(0), _walk(1)),
        as_of=NOW,
    )
    assert report.resolved_decision_count == 2
...
def test_structural_failure_blocks_certification():
    report = build_comprehensive_decision_intelligence_certification(
        as_of=NOW,
        statistical_report=None,
