...
from providers.massive_options import MassiveOptionBar, MassiveOptionDefinition, MassiveOptionSelection
...
class _UnavailableAlpaca:
    configured = False


class _UnavailableTradier:
    configured = False
...
class _BudgetAwareFallback:
    ...
    def select_contracts(self, *_args, **kwargs):
        ...
        bar = MassiveOptionBar(
            ...
            close=12.7,
            volume=120.0,
            source_identifier="massive-opra-bar:O:SPY261010C00650000:2026-08-10T20:00:00+00:00",
        )
...
def _provider(fallback):
    return RedundantOptionsProvider(
        primary=_UnavailableAlpaca(),
        secondary=_UnavailableTradier(),
        fallback=fallback,
    )
...
def test_massive_fallback_never_silently_truncates_multi_expiration_discovery() -> None:
    ...
    with pytest.raises(RedundantOptionsError, match="opportunity-complete"):
        _provider(fallback).select_contracts(
            "SPY",
...
def test_massive_fallback_remains_available_for_explicit_single_expiration_probe() -> None:
    fallback = _BudgetAwareFallback()
    selections = _provider(fallback).select_contracts(
        "SPY",
        underlying_price=640.0,
        as_of=AS_OF,
        minimum_days_to_expiry=30,
...
def test_legacy_databento_progress_stage_publishes_provider_neutral_name(tmp_path) -> None:
    ...
    _request, created = request_manual_cio_diagnostic(requested_by="test", now=AS_OF, values=values)
    assert created is True
    assert claim_manual_cio_diagnostic(now=AS_OF, values=values) is not None
...
def test_render_blueprint_declares_massive_options_secret() -> None:
    ...
    assert "- key: CAPITAL_INTELLIGENCE_MASSIVE_OPTIONS_API_KEY\n        sync: false" in blueprint
