...
def test_every_root_command_has_exactly_one_classification() -> None:
    ...
    assert report == {
        "ready": True,
        "root_script_count": 109,
        "classified_script_count": 109,
        "missing": [],
        "extra": [],
...
def test_render_runtime_matches_canonical_manifest_behaviorally() -> None:
    ...
    assert command_tokens("render", manifest)[-1] == (
        "run_render_service_workspace.py"
    )
    runtime_active = manifest["root_script_inventory"]["runtime_active"]
    ...
    assert "run_stage_isolated_evidence_pipeline.py" in runtime_active
    assert "run_stage_isolated_evidence_stage.py" in runtime_active
