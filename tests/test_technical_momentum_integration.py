...
def test_multi_engine_cycle_persists_six_results_without_changing_contract(
    tmp_path,
) -> None:
...
def test_personal_cio_adds_technical_context_without_changing_action() -> None:
    technical = TechnicalMomentumEngine(
        FakeTechnicalMomentumProvider(),
        clock=lambda: AS_OF,
    ...
    brief = build_personal_cio_brief(
        ...
        generated_at=NOW,
        analytical_results=(technical,),
    )

    assert brief.action_status is ActionStatus.NO_ACTION
    assert "Technical and momentum" in brief.why_it_matters
    assert "Technical transmission" in brief.portfolio_effect
    assert technical.identifier in brief.evidence_identifiers
