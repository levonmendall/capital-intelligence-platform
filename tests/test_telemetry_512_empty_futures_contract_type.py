...
def test_empty_single_index_retries_untyped_and_rejects_explicit_combo() -> None:
    provider, getter = _provider(
        [
            _FakeResponse(200, {"results": []}),  # strict date query
            _FakeResponse(200, {"results": []}),  # active + trade window + type=single
            _FakeResponse(200, {"results": []}),  # product index + type=single
            _FakeResponse(
                200,
                {
                    "results": [
