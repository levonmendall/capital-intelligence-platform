...
def snapshot(identifier: str = "portfolio:core:1", *, cash: float = 25_000) -> CanonicalPortfolioSnapshot:
...
def test_store_is_append_only_and_returns_latest_complete_state(tmp_path) -> None:
    ...
    assert store.append(snapshot()) == 1
    later = replace(snapshot("portfolio:core:2", cash=20_000), as_of=NOW + timedelta(days=1))
    store.append(later)
    store.verify_integrity()
    assert store.latest("core") == later
    assert store.list_latest() == (later,)
    assert len(store.history("CORE")) == 2
    details = snapshot_details(later, history=store.history("CORE"))
...
def test_legacy_database_migrates_read_only_into_canonical_store(tmp_path) -> None:
    ...
    with sqlite3.connect(legacy) as connection:
        connection.executescript("""
            ...
            INSERT INTO mandates VALUES (1, 'CORE', 'Core', 'standard', 100000, 50000, 101000);
            INSERT INTO holdings VALUES ('CORE', 'SPY', 100, 500, 510, '2026-07-26T12:00:00+00:00');
            INSERT INTO trades VALUES (1, '2026-07-26T12:00:00+00:00', 'CORE', 'BUY', 'SPY', 100, 500, 50000, 'migration');
        """)
    canonical = tmp_path / "canonical.db"
    assert migrate(legacy_path=legacy, canonical_path=canonical, as_of=NOW) == 1
    state = SQLiteCanonicalPortfolioStore(canonical).latest("CORE")
    assert state is not None
    assert state.nav == 101_000
