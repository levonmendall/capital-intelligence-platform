...
def test_portfolio_surface_does_not_present_short_term_aspiration_as_portfolio_state() -> None:
    source = Path("portfolio_ui_refinement.py").read_text(encoding="utf-8")

    assert "COMPOUNDING ASPIRATION" not in source
    assert "MONTHLY_STRETCH_RATE" not in source
    assert "build_compounding_aspiration" not in source
    assert "compounding_aspiration" not in source
    assert '"Performance vs benchmarks"' in source
    assert '"Total P&L"' in source
