...
def test_retired_today_grid_does_not_wrap_or_mutate_the_active_renderer() -> None:
    header = object()
    story = object()
    streamlit = object()
    app = SimpleNamespace(
...
def test_entrypoints_retain_compatibility_install_after_content_refinement() -> None:
    for path in (Path("app.py"), Path("render_app.py")):
        ...
        assert source.index("today_story_placement_refinement.install(app_impl)") < source.index(
