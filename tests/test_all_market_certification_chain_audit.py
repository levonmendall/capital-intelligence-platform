...
def test_capability_scoped_context_can_be_ready_only_with_independent_certification(
    ...
    monkeypatch.setattr(cio_route, "public_all_market_certification_readonly", lambda values: _certified_payload())
    monkeypatch.setattr(cio_route, "latest_manual_cio_diagnostic", lambda values: diagnostic)
    monkeypatch.setattr(cio_route, "_state_path", lambda settings: tmp_path / "state.json")
    ...
    monkeypatch.setattr(cio_route, "_safe_public_requirement_progress", lambda values: None)
    monkeypatch.setattr(cio_route, "_certification_context_matches", lambda *args, **kwargs: (True, True))
...
def test_all_market_ready_requires_terminal_certified_outcome(monkeypatch, tmp_path: Path) -> None:
    ...
    now = datetime(2026, 8, 23, 12, 0, tzinfo=timezone.utc)
    diagnostic = SimpleNamespace(
        state="completed", requested_by="render-release:abc123", request_id="diag-1",
        requested_at=now, started_at=now, completed_at=now,
        progress_stage="six_specialist_committee_cio_cycle", progress_metrics=(),
        progress_recorded_at=now, cycle_key="cycle-1", snapshot_identifier="snapshot-1",
        detail="complete",
    )
    monkeypatch.setattr(cio_route, "public_all_market_certification_readonly", lambda values: payload)
    monkeypatch.setattr(cio_route, "latest_manual_cio_diagnostic", lambda values: diagnostic)
    monkeypatch.setattr(cio_route, "_state_path", lambda settings: tmp_path / "state.json")
    monkeypatch.setattr(cio_route, "_load_json", lambda path: {
        "cycle_key": "cycle-1", "decision_as_of": now.isoformat(),
        "comprehensive_discovery_required": True,
        "comprehensive_discovery_scope_state": "capability_scoped",
    })
    monkeypatch.setattr(cio_route, "_latest_context_attempt", lambda settings: {})
    ...
    for path in producers:
        ...
        for node in ast.walk(tree):
            ...
            if "progress" not in func_name:
                continue
            candidates = list(node.args) + [kw.value for kw in node.keywords if kw.arg == "progress_stage"]
            for candidate in candidates:
                if isinstance(candidate, ast.Constant) and isinstance(candidate.value, str):
                    if candidate.value.startswith("production_context_"):
                        emitted.add(candidate.value)
    assert emitted
    assert emitted <= _PROGRESS_STAGES, f"unregistered progress stages: {sorted(emitted - _PROGRESS_STAGES)}"
