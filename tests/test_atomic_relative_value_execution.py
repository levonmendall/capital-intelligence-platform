...
from portfolio.multi_asset_execution import MultiAssetExecutionStatus, MultiAssetOrderStatus
...
def _snapshot(identifier: str, *, as_of=NOW) -> CanonicalPortfolioSnapshot:
...
def _expression(*, certified: bool = True) -> RelativeValueCandidateExpression:
    return RelativeValueCandidateExpression(
        ...
        thesis="Relative-value pair with defined-risk short implementation.",
...
def _profiles():
    return {
        "LONGETF": SimpleNamespace(instrument_identifier="exec:long", defined_risk=False),
        "INVETF": SimpleNamespace(instrument_identifier="exec:inverse", defined_risk=True),
    }
...
class _DummyExecutor:
    ...
    def __init__(self, *, session_provider=None, quote_provider=None, store=None, portfolio_store=None, universe_store=None, policy=None):
    ...
    def execute(self, **kwargs):
        ...
        if self.next_batch is None:
            raise AssertionError("test batch not configured")
        return self.next_batch


def _batch(snapshot: CanonicalPortfolioSnapshot, *, incomplete: bool = False):
    bad_status = object()
    return SimpleNamespace(
        identifier="batch:1",
        status=MultiAssetExecutionStatus.HELD if incomplete else MultiAssetExecutionStatus.COMPLETED,
        order_results=(
            SimpleNamespace(
                status=bad_status if incomplete else MultiAssetOrderStatus.FILLED,
                requested_base_amount=25_000.0,
                filled_base_amount=12_500.0 if incomplete else 25_000.0,
            ),
            SimpleNamespace(
        ...
        reconciliation=SimpleNamespace(
            reconciled=not incomplete,
            accounting_reconciled=not incomplete,
        ),
...
def _orchestrator(tmp_path, monkeypatch, *, batch):
    ...
    real_store = SQLiteCanonicalPortfolioStore(tmp_path / "portfolio.db")
    beginning = _snapshot("portfolio:before", as_of=NOW - timedelta(minutes=1))
    real_store.append(beginning)
    base = _DummyExecutor(
        portfolio_store=real_store,
        universe_store=object(),
        policy=SimpleNamespace(reconciliation_tolerance=1e-6),
    )
    _DummyExecutor.next_batch = batch
    audit = SQLiteAtomicRelativeValueExecutionStore(tmp_path / "atomic.db")
    return AtomicRelativeValuePaperExecutionOrchestrator(base_executor=base, store=audit), beginning, real_store, audit


def test_complete_expression_commits_exactly_one_canonical_snapshot(tmp_path, monkeypatch):
    orchestrator, beginning, real_store, audit = _orchestrator(
        tmp_path,
        monkeypatch,
        batch=_batch(_snapshot("temporary:end")),
    )
    ...
    assert real_store.latest("COMPOUNDING").identifier == result.ending_snapshot_identifier
    assert result.ending_snapshot_identifier != beginning.identifier
    assert audit.verify_integrity() is True


def test_incomplete_leg_discards_all_provisional_fills(tmp_path, monkeypatch):
    orchestrator, beginning, real_store, _audit = _orchestrator(
        tmp_path,
        monkeypatch,
        batch=_batch(_snapshot("temporary:held"), incomplete=True),
    )
    result = orchestrator.execute(
        expression=_expression(),
        construction=_construction(),
    ...
    assert result.status is AtomicRelativeValueExecutionStatus.HELD
    assert result.canonical_state_changed is False
    assert real_store.latest("COMPOUNDING").identifier == beginning.identifier


def test_naked_short_sell_is_rejected_before_simulation(tmp_path, monkeypatch):
    orchestrator, beginning, _real_store, _audit = _orchestrator(
        tmp_path, monkeypatch, batch=_batch(_snapshot("temporary:end"))
    )
    ...
    profiles = {
        ...
        "SHORTETF": SimpleNamespace(instrument_identifier="exec:short", defined_risk=False),
    }
...
def test_uncertified_expression_cannot_execute(tmp_path, monkeypatch):
    orchestrator, beginning, _real_store, _audit = _orchestrator(
        tmp_path, monkeypatch, batch=_batch(_snapshot("temporary:end"))
    )
...
def test_prepared_commit_recovers_without_duplicate_append(tmp_path, monkeypatch):
    orchestrator, beginning, real_store, audit = _orchestrator(
        tmp_path, monkeypatch, batch=_batch(_snapshot("temporary:end"))
    )
    expression = _expression()
    ...
    audit.append(
        AtomicRelativeValueExecutionAttempt(
            ...
            reasons=("prepared before crash",),
            attempt=1,
            canonical_state_changed=False,
        )
    )
    real_store.append(_snapshot(committed_id))
    result = orchestrator.execute(
    ...
    assert real_store.latest("COMPOUNDING").identifier == committed_id
    assert audit.verify_integrity() is True
