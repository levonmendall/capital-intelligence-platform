...
Causal calibration must never accept a hindsight label against an unknown or
non-transmission edge. This is the canonical writer for empirical causal outcomes: it
verifies that the exact predicted ``transmits_to`` edge exists, and that the realized
observation occurs strictly after the edge's decision-time timestamp, before delegating
to the append-only causal store.
"""
...
def append_point_in_time_causal_outcome(
    ...
    if edge is None:
        raise CausalOutcomeResolutionError(
            "causal outcome references an unknown predicted edge"
        )
    if str(edge.get("relationship", "")) != "transmits_to":
        raise CausalOutcomeResolutionError(
            "only predicted transmission edges can receive causal outcomes"
        )
