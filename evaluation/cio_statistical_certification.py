...
This module turns append-only Decision Intelligence outcomes into evidence about whether
the CIO process has actually improved terminal portfolio wealth versus cash and the
best governed alternative.  It deliberately separates *measurement* from *promotion*:
no metric here changes thresholds, specialist weights, construction policy, or capital
authority.

Certification is intentionally demanding.  A small paper sample may be informative,
but it cannot support a platform performance claim or automatic policy promotion.
"""
...
def certify_walk_forward(observations: Iterable[WalkForwardObservation]) -> WalkForwardIntegrityReport:
    ...
    passed = not (future or training or provider or outcomes)
...
def _correlation(left: Sequence[float], right: Sequence[float]) -> float | None:
    ...
    denominator = sqrt(sum((a - ml) ** 2 for a in lrank) * sum((b - mr) ** 2 for b in rrank))
    return None if denominator <= 1e-12 else round(numerator / denominator, 8)


def _rows(pairs: Iterable[tuple[Mapping[str, Any], Mapping[str, Any]]]) -> tuple[DecisionValidationRow, ...]:
    result = []
    for packet, outcome in pairs:
        ...
        expected_candidate = float(opportunity["candidate_expected_return"])
        expected_improvement = float(opportunity["marginal_portfolio_improvement"])
        ...
        result.append(
            DecisionValidationRow(
                ...
                expected_candidate_return=expected_candidate,
                realized_candidate_return=realized_candidate,
                expected_portfolio_improvement=expected_improvement,
...
def build_cio_statistical_certification(
    *,
    ...
    rows = _rows(decision_outcome_pairs)
    if not rows:
    ...
    # Candidate-level expected improvement is compared with its realized opportunity
    # edge, scaled by the allocation change when the packet provides that change.
    improvement_errors = []
    for (packet, outcome), row in zip(tuple(decision_outcome_pairs), rows):
        opportunity = dict(packet["opportunity"])
        weight_change = float(opportunity["proposed_target_weight"]) - float(opportunity["current_weight"])
        realized_candidate_edge = float(outcome["realized_candidate_return"]) - float(outcome["realized_best_alternative_return"])
        improvement_errors.append(abs(row.expected_portfolio_improvement - weight_change * realized_candidate_edge))
    improvement_mae = fmean(improvement_errors)
    asset_classes = {row.asset_class for row in rows}
    gates = (
        ...
        ("asset_class_breadth", len(asset_classes) >= resolved_policy.minimum_asset_classes),
    ...
    certified = all(passed for _name, passed in gates)
    return CIOStatisticalCertificationReport(
        ...
        gates=gates,
        statistically_certified=certified,
    )
